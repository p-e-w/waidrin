...
import { Box, Flex, Link, Text, TextField, RadioGroup, Select, Button, ScrollArea, Card, Checkbox } from "@radix-ui/themes";
...
export default function ConnectionSetup({ onNext, onBack }: { onNext?: () => void; onBack?: () => void }) {
  ...
  const canFetchModels = connectionType === "openrouter" && 
    openRouterConfig.apiKey && 
    validateApiKey(openRouterConfig.apiKey);
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      console.log('Fetching models with config:', { ...config, apiKey: config.apiKey.substring(0, 8) + '...' });
            ...
            <>
                  ...
                  {availableModels.length > 0 && (
                    <Box className="mt-1 mb-3">
                      <TextField.Root
                        ...
                        className="mb-3"
                      />
                      <Text size="4" color="red" style={{ display: 'block', marginBottom: '12px', padding: '8px', backgroundColor: 'var(--red-2)', border: '1px solid var(--red-6)', borderRadius: '4px' }}>
                        <strong>⚠️ IMPORTANT:</strong> For Waidrin to work as intended, please select a model that supports <code>structured_outputs</code>. Models without this feature may not work properly with Waidrin.
                      </Text>
                    </Box>
                  )}
                  
                  {availableModels.length === 0 ? (
                    <Box className="mt-1 p-4 border border-gray-300 rounded" style={{ minHeight: '120px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Text size="3" color="gray">
                        {fetchStatus === 'idle' ? 'Enter your API key and click "Fetch Models" to see available models' : 
                         fetchStatus === 'loading' ? 'Loading models...' :
                         fetchStatus === 'error' ? 'Failed to load models' :
                         'No models available'}
                      </Text>
                    </Box>
                        ...
                        {filteredModels.map((model: StandardizedModelType) => (
                          <Card
                            ...
                            style={{
                              ...
                              cursor: 'pointer',
                              border: openRouterConfig.selectedModel === model.id ? '2px solid var(--accent-9)' : '1px solid var(--gray-6)',
                              background: openRouterConfig.selectedModel === model.id ? 'var(--accent-2)' : 'var(--gray-1)',
                              width: 'calc(100% - 4px)',
                          ...
                          >
                            <Flex direction="column" gap="1" style={{ width: '100%', minWidth: 0 }}>
                              <Flex justify="between" align="center" style={{ width: '100%', minWidth: 0 }}>
                                <Text weight="medium" size="3" style={{ wordBreak: 'break-word', minWidth: 0, flex: 1 }}>{model.name}</Text>
              ...
              <Box>
                <Text size="5" color="blue">
                  <strong>OpenRouter:</strong> Access to a wide variety of AI models through a single API. You'll need an API key from{" "}
                  <Link href="https://openrouter.ai">OpenRouter</Link> to get started.
                </Text>
              </Box>
            </>
          )}
